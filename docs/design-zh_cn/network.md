...
本篇整体介绍一下网络库组件， 大部分是参考muduo库的设计。
如果熟悉muduo库或者http相关的中间件， 那么阅读起来相对比较轻松。

建议:
略读底层`socket`,`inetaddress`, `net_utils`这些C++化和RAII化的封装， 熟悉C语言网络编程这部分会很轻松。
先阅读`Channel`->`Poller`->`Epoller`->`EventLoop`
先关注LT模式的实现方式，暂时忽略定时器， ET模式的实现。
重点是`Buffer`, 理解这个设计配合上层， 明白如何解决粘包问题。
然后看wake_fd跨线程唤醒， 等待队列。 (EventLoop)
看读写逻辑`TcpConnection`， `sock_fd`一次完整的周期， 一次完整的TCP连接。
读TcpServer， Acceptor， 看单Reactor是怎么跑起来的， 编写一个echo server
多Reactor 线程池， 主loop sub_loop, io_loop. 职责分工。跨线程问题怎么解决的。

补ET模式, 定时器实现， 彻底理解IO多路复用 + 非阻塞模型。

...... 后续http层。

...... 扩展: 异步IO模型, io_uring. (Proactor)
...
class Socket : public NonCopyable {   // 不可拷贝
  const int sockfd_;                  // const：构造后不会换 fd
public:
  explicit Socket(int sockfd);
  ~Socket();                          // 析构时 close(sockfd_)
};

这里面的fd涵盖了多种意义的fd, 比如服务端监听的fd, 客户端连接accept的fd， 或者wake_fd(用来唤醒epoll_wait), timer_fd(一次性唤醒/周期性唤醒的fd)。

某些操作只针对特定的fd。 
真正持有fd完整生命周期的只有`Socket.h`, 另外的`TcpConnection.h`持有客户端连接的`Socket`对象， `Acceptor`持有服务端监听的`Socket`对象。
`EventLoop`持有定时器的timer_fd, wake_fd. 
`Channel`负责描述fd和设置执行各种回调， `Poller`负责底层系统调用监听fd状态和准备就绪链表。
...
这些负责把 “某个 fd 上发生了什么事件” 抽象成统一的数据结构和回调接口。
理解: `Channel`只负责描述对象和路由事件，同步内核。
Channel Tie 防止悬空回调， 更改状态并与底层状态的一致性同步。
...
这一部分的难点在于理解Event Loop到底在干什么， 跨线程问题怎么来的以及怎么避免出错的。 
